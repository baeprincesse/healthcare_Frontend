...
export default function Dashboard() {
    ...
    const fetchData = async () => {
      try {
        const [apptRes, recRes] = await Promise.all([
          ...
          api.get('/medical-records/patient').catch(() => ({ data: { records: [] } })),
        ]);
  ...
  const prescriptionsCount = medicalRecords.filter((r) => r.prescription).length
  ...
  const stats = [
    ...
    { label: 'Prescriptions', value: prescriptionsCount, icon: Pill, tint: 'bg-amber-50 text-amber-600' },
    { label: 'Medical Records', value: medicalRecords.length, icon: FileText, tint: 'bg-indigo-50 text-indigo-600' },
  ]
              ...
              {appointments.slice(0, 5).map((item) => (
                <div key={item.id} className="flex flex-col gap-4 rounded-xl border border-[#E7ECE9] p-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {item.consultationType === 'ONLINE' && item.jitsiMeetingUrl && item.status === 'IN_PROCESS' && (
